...
export default function ProjectCard({ project }: { project: Project }) {
  ...
  const { data: images, error, loading } = useProjectData<CarouselImage[]>("projects-details", undefined, display, tag);
  ...
  return (
    <div className="project_card_container">
      <div className="laptop_preview">
        <LaptopPreviewModal project={project} onOpen={open} laptopRef={laptopRef}/>
      </div>

      <div className="mobile_preview">
        <MobilePreviewModal project={project} onOpen={open} mobileRef={mobileRef}/>
      </div>
        ...
        <p className="px-4">{project.description}</p>
          <span className="flex gap-2 p-4">
            ...
            {project.stack.map((s, i) => (
              <Image key={i} title={s.alt} src={s.icon} alt={s.alt} width={24} height={24}/>
            ))}
        </span>
      </div>
